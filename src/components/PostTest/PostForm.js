...
const PostForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:5000/poststudents', studentData);
    ...
    return (
        <div className="bg-gray-300">
            <h2 className="absolute ml-12 mt-7 text-2xl">Post-Test Questionnaire</h2>
            <div className="min-h-screen flex items-center justify-center bg-gray-300">

                {/* Settings Button */}
                <button
                    onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                    className="absolute mt-2 top-4 right-4 p-3 bg-gray-800 text-white rounded-full focus:outline-none"
                >
                    ⚙️
                </button>
                ...
                <div className="bg-gray-100 p-8 rounded-lg shadow-2xl max-w-xl w-full pb-20">
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col items-center space-y-6">
                        ...
                        {/* Submit Button */}
                        <button type="submit" className="w-1/2 p-2 bg-purple-600 hover:bg-blue-600 text-white rounded-md">
                            Start Quiz
                        </button>
                    </form>
                </div>
