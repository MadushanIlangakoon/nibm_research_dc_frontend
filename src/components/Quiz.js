...
const Quiz = () => {
    ...
    useEffect(() => {
        ...
        if (stream) {
            axios
                .get(`http://localhost:5000/questions?stream=${stream}`)
                .then((response) => {
    ...
    useEffect(() => {
        ...
        if (questions.length > 0 && currentQuestionIndex < questions.length && !isRecording) {
        ...
        try {
            ...
            const uploadVideo = async (blob, questionId) => {
                try {
                    ...
                    const videoFile = new File([blob], `${questionId}.webm`, { type: 'video/webm' });
                    formData.append("video", videoFile);
                    formData.append("question_id", questionId);

                    const response = await axios.post("http://localhost:5000/upload-video-new", formData, {
    ...
    const handleSubmit = async () => {
        ...
        if (submittedQuestions.includes(currentQuestionId)) {
            console.log(`Question ${currentQuestionId} already answered. Skipping submission.`);
    ...
    const handleAutoSubmit = async () => {
        ...
        // Check if this question has already been submitted
        if (submittedQuestions.includes(currentQuestionId)) {
            console.log(`Question ${currentQuestionId} already auto-submitted. Skipping.`);
            return; // Prevent duplicate submission
        }
        ...
        const autoAnswer = {
            question_id: currentQuestionId,
            answer: currentAnswer, // Send the typed but unsubmitted answer as empty
            elapsed_time: 60,
        };

        try {
            const responseAuto = await axios.post("http://localhost:5000/answers", {
                answers: [autoAnswer],
            });
    ...
    return (
        <div className="min-h-screen bg-gray-800 flex flex-col justify-center items-center p-4">
            {/*<h2 className="text-3xl font-bold text-white mb-6">Quiz</h2>*/}
            <div className="w-full max-w-md h-[60vh] bg-white p-8 rounded-lg shadow-xl">
                ...
                {questions.length > 0 ? (
                    <div className="mb-8">
                        ...
                        <p className="text-xl text-gray-700 font-semibold mb-4 mt-4">{questions[currentQuestionIndex]?.question_text}</p>
                ...
                <div className="flex justify-end">
                    <button
                        ...
                        className={`w-1/3 py-3 font-bold text-md rounded-lg transition ${isSubmitDisabled ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 text-white hover:bg-blue-700"}`}
